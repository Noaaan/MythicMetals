 ...
 *
 * Please refer to https://github.com/TechReborn/TechReborn/blob/ef775af6b88a464045d1bcb6e7c1d6334c054fdd/src/main/java/techreborn/config/TechRebornConfig.java
 *
...
public class ConfigHandler
{
    ...
    private HashMap<Field, IConfig> getConfigFields() {
        final HashMap<Field, IConfig> fieldMap = new HashMap<>();
        for (Field field : clazz.getDeclaredFields()) {
            if (!field.isAnnotationPresent(IConfig.class)) {
                continue;
            }
            if (!Modifier.isStatic(field.getModifiers())) {
                throw new UnsupportedOperationException("Config field must be static");
            }
            IConfig annotation = field.getAnnotation(IConfig.class);
    ...
    public HashMap<String, JsonObject> toJson() {
        ...
        for (Map.Entry<Field, IConfig> entry : fieldMap.entrySet()) {
            ...
            IConfig annotation = entry.getValue();
