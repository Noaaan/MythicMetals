...
- Renamed and moved a bunch of recipes
  - This is a result of a lot of the recipes needing to be updated for 1.21.4
  - Most recipes are now generated by code.
  - There might be edge cases or errors with this, so please report any recipe problems should you find any!
- Renamed `star_platinum` to `star_platinum_ingot` for consistency.
  - The language in-game is the same, although this should help + break recipes and tools which relied on this fact
  - Includes a registry alias, so any items you have should be converted
- Renamed the "Prometheum Bouquet" to "Prometheum Rose" to match its new texture
  - Includes a registry alias, so any loose bouquets you have should be converted
- Adjusted stats of the Adamantite Forge Controller
    - Done to comply with Alloy Forgery 3's updates
    - Now uses the `mythicmetals:rare_plus_tier`, which still applies tier 3 recipes
